                          ...
                          VencCbInputBufferDoneInfo* pBufferDoneInfo)
{
    ...
    {
                logv("return this buf(%lu)", pBufferDoneInfo->pInputBuffer->nID);
...
int setEncodeType(EncodeCompContex *p, VideoEncodeConfig* config)
{
    ...
    if(p->mEncodeType == VENC_CODEC_JPEG)
    {
        ...
        bitRateRange.bitRateMax = p->mBitRate;
        bitRateRange.bitRateMin = p->mBitRate;
#endif
#if MPP_ENC_API
...
#if TINA_LINUX_SUPPORT
        VencSetParameter(p->pEncoder, VENC_IndexParamBitrate, &p->mBitRate);
        VencSetParameter(p->pEncoder, VENC_IndexParamFramerate, &p->mFrameRate);
        VencSetParameter(p->pEncoder, VENC_IndexParamSetBitRateRange, &bitRateRange);
#endif
#else
...
#if TINA_LINUX_SUPPORT
        VideoEncSetParameter(p->pEncoder, VENC_IndexParamBitrate, &p->mBitRate);
        VideoEncSetParameter(p->pEncoder, VENC_IndexParamFramerate, &p->mFrameRate);
        VideoEncSetParameter(p->pEncoder, VENC_IndexParamSetBitRateRange, &bitRateRange);
#endif
#endif
    }
