...
static int AwRawStreamSupplyData(RawStreamParser *rawStreamParser)
{
    unsigned char *Buf = rawStreamParser->pRawStreamBuf;
    int nRealLen = 0;
    if(rawStreamParser->nValidSize <= 0)
    {
        nRealLen = CdxStreamRead(rawStreamParser->file, Buf, FRAME_SIZE);
        if(nRealLen <= 0)
            return -1;
        else
        {
            rawStreamParser->nValidSize = nRealLen;
            rawStreamParser->nCurIndex = 0;
        }
    }
    else
    {
        char* tmpBuf = (char*)malloc(rawStreamParser->nValidSize);
        if(tmpBuf == NULL)
        {
            loge("malloc for tmpBuf, size = %d",rawStreamParser->nValidSize);
            return -1;
        }
        memcpy(tmpBuf, (Buf + rawStreamParser->nCurIndex),rawStreamParser->nValidSize);

        int nLen = FRAME_SIZE - rawStreamParser->nValidSize;
        nRealLen = CdxStreamRead(rawStreamParser->file, &(Buf[rawStreamParser->nValidSize]), nLen);
        memcpy(Buf, tmpBuf, rawStreamParser->nValidSize);
        free(tmpBuf);
        tmpBuf = NULL;
        if(nRealLen <= 0)
        {
...
static int AwRawStreamGetDataTrunkSize(RawStreamParser *rawStreamParser)
{
    ...
    if(rawStreamParser->nValidSize <= 0)
    {
        ret = AwRawStreamSupplyData(rawStreamParser);
        if(ret == -1)
            return -1;
    }

    while(!StartCodeFound) {
        pCurDataPtr = (char *)Buf + rawStreamParser->nCurIndex;
        ...
        for(pos = 0; pos < (rawStreamParser->nValidSize - 3); pos++)
        {
            tmpBuf[0] = *(pCurDataPtr + pos);
            tmpBuf[1] = *(pCurDataPtr + pos + 1);
            tmpBuf[2] = *(pCurDataPtr + pos + 2);
        ...
        logv("nFindStartCode = %d, pos = %d, valisSize = %d",\
            StartCodeFound, pos, rawStreamParser->nValidSize);
        if(StartCodeFound == 1)
        {
            rawStreamParser->nCurIndex += pos;
            nStart = pos;
            if(*(pCurDataPtr + pos -1) == 0)
            {
                rawStreamParser->nCurIndex -= 1;
                nStart -= 1;
            }
            StartCodeFound = 0;

            //* find the second startcode
            for(pos += 3; pos < (rawStreamParser->nValidSize - 3); pos++)
            {
                tmpBuf[0] = *(pCurDataPtr + pos);
                tmpBuf[1] = *(pCurDataPtr + pos + 1);
                tmpBuf[2] = *(pCurDataPtr + pos + 2);
                logv("pCurDataPtr = %p, pos = %d, tmpBuf[0] = %x", pCurDataPtr, pos, tmpBuf[0]);
                if(tmpBuf[0] == 0 && tmpBuf[1] == 0 && tmpBuf[2] == 1)
                {
                    StartCodeFound = 1;
                    break;
            ...
            if(StartCodeFound == 1)
            {
                if(*(pCurDataPtr + pos - 1) == 0)
                {
                    rawStreamParser->nRawStreamBufSize =  pos - nStart - 1;
                }
                else
                {
                    rawStreamParser->nRawStreamBufSize = pos - nStart;
                }
            }
            else
            {
                ...
                if(ret == -1)
                {
                    ...
                    {
                        rawStreamParser->nRawStreamBufSize = pos - nStart + 3;
                        logv("======Stream EOS=========");
    ...
    logv(" nStreamDataLen = %d",rawStreamParser->nRawStreamBufSize);
    return rawStreamParser->nRawStreamBufSize;
}
...
static cdx_int32 AwRawStreamParserGetMediaInfo(CdxParserT *parser, CdxMediaInfoT *pMediaInfo)
{
    ...
    {
        if(rawStreamParser->tempVideoInfo.eCodecFormat != VIDEO_CODEC_FORMAT_UNKNOWN)
        {
            CDX_LOGD("set codec format is %d !", rawStreamParser->tempVideoInfo.eCodecFormat);
            pMediaInfo->program[0].video[0].eCodecFormat = rawStreamParser->tempVideoInfo.eCodecFormat;
        }
        else
...
static cdx_int32 AwRawStreamParserRead(CdxParserT *parser, CdxPacketT *pkt)
{
    ...
    cdx_uint8* pCpyPtr = rawStreamParser->pRawStreamBuf + rawStreamParser->nCurIndex;
    if(pkt->length <= pkt->buflen)
    {
        memcpy(pkt->buf, pCpyPtr, pkt->length);
    }
    else
    {
        memcpy(pkt->buf, pCpyPtr, pkt->buflen);
        memcpy(pkt->ringBuf, pCpyPtr + pkt->buflen,
            pkt->length - pkt->buflen);
    }
    rawStreamParser->nCurIndex += pkt->length;
    rawStreamParser->nValidSize -= pkt->length;
...
int AwRawStreamParserInit(CdxParserT *parser)
{
    ...
    logd(" rawStreamParser->file: %p ", rawStreamParser->file);

    cdx_uint32 eVideoCodecFormat = (eStreamType == h265_raw_stream) ? \
                                    VIDEO_CODEC_FORMAT_H265 : VIDEO_CODEC_FORMAT_H264;

    ProbeVideoSpecificData(&rawStreamParser->tempVideoInfo,
                            rawStreamParser->pRawStreamBuf,
                            AW_RAW_STREAM_PROB_SIZE, eVideoCodecFormat,
