...
#define DEV_DISPLAY_PATH "/sys/class/disp/disp/attr/sys"
...
{
	const char* TPdevName = "axp2202-pek";
	int i = 0;
...
static void *scanPowerKey(void *arg)
{
	...
	int display_fd = open(DEV_DISPLAY_PATH, O_RDONLY | O_NONBLOCK);
	if(read(display_fd, buffer, sizeof(buffer)) > 0){
		display_state = 0;
	}
	else{
		display_state = 1;
		...
		nevents = epoll_wait(epollfd, events_inputs, input_event_count, timeout);
			for(i = 0; i < 2; i++){
				int err = read(fd, key_event, sizeof(struct input_event));
				 ...
				 if(debugfs_node == 0){
				 	sysret = system("mount -t debugfs none /sys/kernel/debug");
				 	debugfs_node = 1;
				 }
				 ...
				 if(key_event->value == 0 && key_event->type == 1 && key_event->code == KEY_POWER){
				 	if(display_state == 0){
				 		...
				 		if(suspend_time_ms - press_time < 500){
				 			sysret = system("echo suspend > /sys/kernel/debug/dispdbg/command");
				 			display_state = 1;
				 		}
				 	}
				 	else if(display_state == 1){
				 		wakeup_time_ms = curr_time_ms();
				 		if(wakeup_time_ms - press_time < 500){
				 			sysret = system("echo resume > /sys/kernel/debug/dispdbg/command");
				 			display_state = 0;
				 		}
				 	}
				 sysret = system("echo disp0 > /sys/kernel/debug/dispdbg/name");
				 sysret = system("echo 1 > /sys/kernel/debug/dispdbg/start");
				}
			}
	}
	free(key_event);
