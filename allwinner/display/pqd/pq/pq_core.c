    ...
    switch (head->id) {
        case HARDWARE_INFO: {
            if (packet->cmd == CMD_GET) {
                ...
                int id, version, gamma_bitwidth;
                pqal_->get_hardware_info(&id, &version, &gamma_bitwidth);
                hwinfo->ic_code = id;
                hwinfo->de_version = version;
		hwinfo->gamma_bitwidth = gamma_bitwidth;
        ...
        case DCI_CTRL:
            if (packet->cmd == CMD_GET) {
                char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process DCI_CTRL read");
                pqal_->get_dci(data, size);
                request->base.ack_callback((const char*)packet, packet->size);
            } else {
                const char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process DCI_CTRL write");
                pqal_->set_dci(data, size);
                struct pq_package* reply = create_reply_packet(CMD_SET, 0);
                request->base.ack_callback((const char*)reply, reply->size);
                const struct hardware_data* hwdata = pq_hardware_data();
                int index = pqal_->get_packet_store_index(head->id, data, size);
        ...
        case DEBAND_CTRL:
            if (packet->cmd == CMD_GET) {
                char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process DEBAND_CTRL read");
                pqal_->get_deband(data, size);
                request->base.ack_callback((const char*)packet, packet->size);
            } else {
                const char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process DEBAND_CTRL write");
                pqal_->set_deband(data, size);
                struct pq_package* reply = create_reply_packet(CMD_SET, 0);
                request->base.ack_callback((const char*)reply, reply->size);
                const struct hardware_data* hwdata = pq_hardware_data();
                int index = pqal_->get_packet_store_index(head->id, data, size);
        ...
        case DE35X_SHARP_CTRL:
            if (packet->cmd == CMD_GET) {
                char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process DE35X_SHARP_CTRL read");
                pqal_->get_peak(data, size);
                request->base.ack_callback((const char*)packet, packet->size);
            } else {
                const char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process DE35X_SHARP_CTRL write");
                pqal_->set_peak(data, size);
                struct pq_package* reply = create_reply_packet(CMD_SET, 0);
                request->base.ack_callback((const char*)reply, reply->size);
                const struct hardware_data* hwdata = pq_hardware_data();
        ...
        case SNR_CTRL:
            if (packet->cmd == CMD_GET) {
                char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process SNR_CTRL read");
                pqal_->get_snr(data, size);
                request->base.ack_callback((const char*)packet, packet->size);
            } else {
                const char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process SNR_CTRL write");
                pqal_->set_snr(data, size);
                struct pq_package* reply = create_reply_packet(CMD_SET, 0);
                request->base.ack_callback((const char*)reply, reply->size);
                const struct hardware_data* hwdata = pq_hardware_data();
                int index = pqal_->get_packet_store_index(head->id, data, size);
        ...
        case GTM_CTRL:
            if (packet->cmd == CMD_GET) {
                char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process GTM_CTRL read");
                pqal_->get_gtm(data, size);
                request->base.ack_callback((const char*)packet, packet->size);
            } else {
                const char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process GTM_CTRL write");
                pqal_->set_gtm(data, size);
                struct pq_package* reply = create_reply_packet(CMD_SET, 0);
                request->base.ack_callback((const char*)reply, reply->size);
                const struct hardware_data* hwdata = pq_hardware_data();
                int index = pqal_->get_packet_store_index(head->id, data, size);
        ...
        case TNR_CTRL:
            if (packet->cmd == CMD_GET) {
                char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process TNR_CTRL read");
                pqal_->get_tnr(data, size);
                request->base.ack_callback((const char*)packet, packet->size);
            } else {
                const char* data = packet->data;
                int size = packet->size - sizeof(struct pq_package);
                dlogd("start process TNR_CTRL write");
                pqal_->set_tnr(data, size);
                struct pq_package* reply = create_reply_packet(CMD_SET, 0);
                request->base.ack_callback((const char*)reply, reply->size);
                const struct hardware_data* hwdata = pq_hardware_data();
                int index = pqal_->get_packet_store_index(head->id, data, size);
