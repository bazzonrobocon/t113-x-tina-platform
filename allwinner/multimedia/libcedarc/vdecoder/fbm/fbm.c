...
nWidth=%d, nHeight=%d nAlignStride = %d",
    ...
    if(pFbmCreateInfo->nBufferType == BUF_TYPE_ONLY_REFERENCE && \
       pFbmCreateInfo->eCodecFormat == VIDEO_CODEC_FORMAT_H264)
    ...
    pFrameNode = (FrameNode*)malloc((p->nMaxFrameNum + p->nExtraBufNum)*sizeof(FrameNode));
    if(pFrameNode == NULL)
    {
        loge("memory alloc fail, alloc %d bytes.", (int)((p->nMaxFrameNum + p->nExtraBufNum)*sizeof(FrameNode)));
        free(p);
        return NULL;
    }
    memset(pFrameNode, 0, (p->nMaxFrameNum + p->nExtraBufNum)*sizeof(FrameNode));
    ...
    for(i=0; i<(p->nMaxFrameNum + p->nExtraBufNum); i++)
    {
        ...
        if((pFbmCreateInfo->bGpuBufValid==0 || \
            pFbmCreateInfo->nBufferType==BUF_TYPE_ONLY_REFERENCE) && i < p->nMaxFrameNum)
        {
    ...
    if(i < (p->nMaxFrameNum + p->nExtraBufNum))
    {
            ...
            int* nAlignValue, int nWidth, int nHeight)
{
    ...
    switch(ePixelFormat)
    {
	    ...
	    pPicture->nBufFd = CdcGetBufferFd(pFbm->memops, pMem);
            ...
            pMem = (char*)malloc(sizeof(struct sunxi_metadata));
            if(pMem == NULL)
            {
                loge("memory alloc fail, require %d bytes for picture metadata buffer", sizeof(struct sunxi_metadata));
                return -1;
            }
            memset(pMem, 0, sizeof(struct sunxi_metadata));
            CdcMemFlushCache(pFbm->memops, pMem, sizeof(struct sunxi_metadata));
...
int FbmDynamicMallocFbmBuf(Fbm* pFbm)
{
    FrameNode* pFrameNode;

    pFrameNode = &pFbm->pFrames[pFbm->nMaxFrameNum];
    if(!pFbm->bThumbnailMode)
    {
        logw("dynamic call allocate pic buf, i = %d.",pFbm->nMaxFrameNum);
        if(FbmAllocatePictureBuffer(pFbm, \
           &pFrameNode->vpicture, &pFbm->nAlignValue, pFrameNode->vpicture.nWidth, pFrameNode->vpicture.nHeight) != 0)
        {
            loge("allocate dynamic fbm buffer error!");
            return -1;
        }
    }
    else
    {
        //* thumbnail mode, other pictures share picture buffer with the first one.
        pFrameNode->vpicture.pData0 = pFbm->pFrames[0].vpicture.pData0;
        pFrameNode->vpicture.pData1 = pFbm->pFrames[0].vpicture.pData1;
        pFrameNode->vpicture.pData2 = pFbm->pFrames[0].vpicture.pData2;
        pFrameNode->vpicture.pData3 = pFbm->pFrames[0].vpicture.pData3;
        pFrameNode->vpicture.phyYBufAddr = pFbm->pFrames[0].vpicture.phyYBufAddr;
        pFrameNode->vpicture.phyCBufAddr = pFbm->pFrames[0].vpicture.phyCBufAddr;
    }
    pFrameNode->vpicture.nColorPrimary = 0xffffffff;
    FbmEnqueue(&pFbm->pEmptyBufferQueue, pFrameNode);
    pFbm->nEmptyBufferNum++;
