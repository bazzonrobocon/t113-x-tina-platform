...
status_t M3u8Parse(const void *_data, cdx_uint32 size, Playlist **P, const char *baseURI)
{
    *P = NULL;
    ...
    while (offset < size)
    {
        ...
        if (startsWith(line.mData,"#EXT-X-MAP:"))
        {
            PlaylistItem *item = (PlaylistItem*)malloc(sizeof(PlaylistItem));
            if (item == NULL)
            {
            ...
            memcpy(&(item->itemMeta), &itemMeta, sizeof(AMessage));
            seqNum++;
            char *tmp_str = strchr(line.mData,'"');
            char *tmp_str2= strchr(tmp_str + 1,'"');
            char *url = (char*)malloc(tmp_str2-tmp_str);
            if (url == NULL)
            {
                CDX_LOGE("err_no_memory");
                err = err_no_memory;
            ...
            strncpy(url, tmp_str+1, tmp_str2-tmp_str-1);
            url[tmp_str2-tmp_str-1] = '\0';
            MakeURL(playList->mBaseURI.mData, url, &item->mURI);
            item->next = NULL;
            if (playList->mItems == NULL)
            {
                playList->mItems = item;
