...
static CDX_U64 readStco(MOVContext *c, MOVStreamContext *st, cdx_uint32 idx)
{
    ...
    if(idx >= st->stco_size)
    {
        CDX_LOGW("type = %d. index<%d> is large than stco_size<%d>, maybe error",st->stsd_type, idx, st->stco_size);
        return INT_MAX;
    }
...
CDX_S16 MovReadSampleFragment(struct CdxMovParser *p)
{
    ...
    int is_netstream = 0;
    int flag = 0;
    if(CdxStreamAttribute(p->stream) & CDX_STREAM_FLAG_NET)
    {
        logv(" the video is network stream");
        is_netstream = 1;
    }
...
//    CDX_LOGD("has_video = %d, has_audio = %d", s->has_video, s->has_audio);
    ...
    flag = is_netstream ? voffset < aoffset:vpts < apts;
    if( (flag) && s->has_video && !v_end)
    {
    ...
    else if((!flag) && s->has_audio && !a_end)
    {
...
CDX_S16 MovReadSample(struct CdxMovParser *p)
{
    ...
    for(i=0; i<c->nb_streams; i++)
    {
        ...
        if((st->stsd_type && !st->read_va_end))
        {
            ...
            //if current stco idx is larger than nums of stco, it indicate that this track is end.
            if(vas_chunk_ost[i] == INT_MAX) {
                CDX_LOGW("Current chunk is out of size! We may met end of this track [%d]!", st->stsd_type);
                st->read_va_end = 1;
            }
        }
    }
