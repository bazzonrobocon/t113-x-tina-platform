...
static int alsaAportInit(void *hdl, int enable, int format, int samplerate,
			 int channels, int samplebits, int bitrate)
{
	aInPort *hdlAudio = (aInPort *)hdl;
	...
	if(config == NULL)
	{
           printf("alsaAportInit malloc fail\n");
           return -1;
	}
...
static int alsaAportGetBitrate(void *hdl)
{
	aInPort *hdlAudio = (aInPort *)hdl;
	if (hdlAudio == NULL)
		return -1;
...
aInPort *CreateAlsaAport(int index)
{
	...
	aport->getAudioBitRate = alsaAportGetBitrate;
