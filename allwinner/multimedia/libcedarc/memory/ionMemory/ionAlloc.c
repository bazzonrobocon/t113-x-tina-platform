...
static int ion_alloc_open(void)
{
    VeOpsS* veOps = NULL;
...
static void ion_alloc_close(void)
{
...
#if DMA_HEAP_ALLOC
    ret = CdcDmaheapAllocFd(g_context->fd, size);
    dma_buf_fd = ret;
#else
...
static int ion_alloc_get_total_size(void)
{
    if(g_context == NULL)
    {
...
static int ion_alloc_setup(void)
{
    return -1;
}

static int ion_alloc_shutdown(void)
{
    return -1;
}

static unsigned int ion_alloc_get_ve_addr_offset(void)
{
    if(g_context != NULL)
