        ...
        else if(ret == 0)
        {
            ...
            if(impl->totalSize > 0)
            {
                ...
                if(streamDLStateIsError(impl) == 1)
                {
                    ...
                    logd("current data pos %lld, reconnect at(%lld/%lld)", pos, impl->readPos + readSize, impl->totalSize);
                    while(1)
                    {
                        startTime = GetNowUs();
                        ret = seekReconnect(impl, impl->readPos + readSize);
                        if(ret < 0)
                        {
                            readSize = -1;
                            goto __exit;
