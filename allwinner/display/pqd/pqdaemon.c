...
int main(int argc, char** argv)
{
    ...
    dlogd("start main debug_is_verbose_mode=%d", debug_is_verbose_mode());
...
{
    if (!sdat)
        dloge("socket_packet_handle null point sdat");
    dlogd("start read position: %d, size=%d, state=%d, p=%p", sdat->read_pos,
            sdat->recv_len, sdat->recv_state, sdat);
    ...
    dlogd("receive socket packet: %zu", count);
...
parse_data:
    switch (sdat->recv_state) {
        ...
        case RECV_START: {
            ...
            dlogd("start parse packet: cmd = %d, size=%d", header.cmd, header.size);
        ...
        case RECV_WAIT_PACKET: {
            ...
            dlogd("get more data remain:%d", sdat->remaining_data_lenght);
            memcpy(d, s, copysize);
            sdat->recv_len -= copysize;
            sdat->read_pos += copysize;
