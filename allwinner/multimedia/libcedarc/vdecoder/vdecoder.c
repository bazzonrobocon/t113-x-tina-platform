...
void DestroyVideoDecoder(VideoDecoder* pDecoder)
{
    ...
    if(p->bInterVe && p->vconfig.veOpsS != NULL)
...
err_exit:
    if (p != NULL) {
        if (p->vconfig.pVeOpsSelf != NULL) {
            p->vconfig.pVeOpsSelf = NULL;
        }
        if (p->vconfig.veOpsS != NULL) {
            p->vconfig.veOpsS = NULL;
        }
        p = NULL;
    }
