...
static int setLayerBuffer(LayerContext* lc,LayerBufMode mode)
{
    ...
    lc->mHoldLastPictureFlag = GetConfigParamterInt("hold_last_picture_flag", 1);
    ...
    for(i=0; i<lc->nGpuBufferCount; i++)
    {
        ...
        if(ret == -1){
            ...
            lc->mHoldLastPicture.nBufFd = tmpStoreHoldLastPictureFd;//* nBufFd of last picture must be fixed.
            if(lc->eDisplayPixelFormat == PIXEL_FORMAT_YUV_MB32_420){
                ...
                if(nGpuBufWidth*nGpuBufHeight<= HOLD_LAST_PICTURE_WITH*HOLD_LAST_PICTURE_HEIGHT && lc->mHoldLastPictureBuf && lc->mTmpCurPicture.pData0){
                    unsigned int bufferSize = nGpuBufWidth * nGpuBufHeight * 3/2;
                    if(lc->nLbcLossyComMod > 0)
                    {
                        bufferSize = getLbcModBuferrSize(lc);
...
static int SetLayerParam(LayerContext* lc, VideoPicture* pPicture,LayerBufMode mode)
{
    ...
    //Use G2D rotation 90Â°/180Â°, (x,y) point offsets need to be calculated.
    ...
    logv("crop_w = %d,crop_h = %d,nWidth = %d,nHeight = %d",vparam.srcInfo.crop_w,vparam.srcInfo.crop_h,pPicture->nWidth,pPicture->nHeight);
...
#if DISPLAY_PICTURE_TIMEDIFF
        long long queue_to_display_end = TPlayerGetNowMs();
	logw("display picture timediff:%lldms\n", (queue_to_display_end - queue_to_display_start));
#endif
    if (!lc->isPlaying) {
...
/* for avoiding fbm signal 11. But still need to deal with it completely*/
static void LayerCleanPictureList(LayerCtrl* l)
{
    LayerContext *lc;
    ...
    while(1) {
        if(lc->pPictureListHeader != NULL) {
            ...
            nodePtr->bUsed = 0; /* clearing the list by marking it 0 */
            i++;
        } else {
            printf("[%s %d]still have %d pic in picture list of layer\n", __func__, __LINE__, i);
            break;
        }
...
double x_ltop = 0.0;
double y_ltop = 0.0;
double x_rbottom = 0.0;
double y_rbottom = 0.0;
double ovl_chn_x  = 0.0;
double ovl_chn_y = 0.0;
int round_ovl_chn_x  = 0;
int round_ovl_chn_y = 0;

#define TP_MIN(i, j) (((i) < (j)) ? (i) : (j))
#define TP_MAX(i, j) (((i) > (j)) ? (i) : (j))

static int LayerCheckDoubleVideoRect(LayerCtrl *l, VideoPicture* pBuf)
{
    ...
    int ovl_flag = 0;
    int ret = 0;
    DOUBLE_VIDEO_LAYER_ERROR_TYPE nDoubleLayerErrType = DOUBLE_VIDEO_LAYER_ERROR_NONE;


    nhlayer = lc->mDispOutPort->getHlayer(lc->mDispOutPort);

    /* disp rect param */
    rect_ch[nhlayer] = lc->mDispOutPort->getChannel(lc->mDispOutPort);
    rect_hlayer[nhlayer] = nhlayer;
    rect_x[nhlayer] = lc->dst_rect.x;
    rect_y[nhlayer] = lc->dst_rect.y;
    rect_width[nhlayer] = lc->dst_rect.width;
    rect_height[nhlayer] = lc->dst_rect.height;

    /* check disp layer overlay or not*/
    if (nhlayer == 1) {
        if (!((rect_x[1] > rect_x[0] + rect_width) ||
            (rect_x[1] + rect_width[1] < rect_x[0]) ||
            (rect_y[1] > rect_y[0] + rect_height[0]) ||
            (rect_y[1] + rect_height[1] < rect_y[0])))
            ovl_flag = 1;
    }

    /* layer input param */
    src_width[nhlayer] = pBuf->nRightOffset - pBuf->nLeftOffset;
    src_height[nhlayer] = pBuf->nBottomOffset - pBuf->nTopOffset;
    if (!src_width[nhlayer])
        src_width[nhlayer] = pBuf->nWidth;
    if (!src_height[nhlayer])
        src_height[nhlayer] = pBuf->nHeight;

    ovl_x[nhlayer] = src_width[nhlayer]/(double)rect_width[nhlayer]*rect_x[nhlayer];
    ovl_y[nhlayer] = src_height[nhlayer]/(double)rect_height[nhlayer]*rect_y[nhlayer];

#if 1 //hlayer 1 check
    if(ovl_flag && nhlayer == 1) {

        if((rect_x[0] + rect_width[0] > lc->nScreenWidth) || (rect_x[1] + rect_width[1] >lc->nScreenWidth)) {
            nDoubleLayerErrType = DOUBLE_VIDEO_LAYER_WIDTH_AREA_ERROR;
            loge("errro, please check your x +disp_width < nScreenWidth!!!");
            ret = -1;
            goto check_done;
        }

        if((rect_x[0]%2 != rect_x[1]%2)) {
            //pBuf->nCoordinateErrFlag = 1;
            nDoubleLayerErrType = DOUBLE_VIDEO_LAYER_COORDINATE_ERROR;
            loge("errro, please check your x or y coordinate odd even!!!");
            ret = -1;
            goto check_done;
        }

        x_ltop = TP_MIN(ovl_x[0],ovl_x[1]);
        y_ltop = TP_MIN(ovl_y[0],ovl_y[1]);
        x_rbottom = TP_MAX(ovl_x[0]+src_width[0], ovl_x[1] + src_width[1]);
        y_rbottom = TP_MAX(ovl_y[0]+src_height[0], ovl_y[1] + src_height[1]);
        ovl_chn_x = x_rbottom - x_ltop;
        ovl_chn_y = y_rbottom - y_ltop;
        round_ovl_chn_x = (int)round(ovl_chn_x);
        round_ovl_chn_y = (int)round(ovl_chn_y);

        if((round_ovl_chn_x%2 != 0) || (round_ovl_chn_y%2 !=0)) {
            nDoubleLayerErrType = DOUBLE_VIDEO_LAYER_DISP_DISLOCATION_ERROR;
            loge("not match the disp cond where ovl_chn_x = %d, ovl_chn_x = %d\n", round_ovl_chn_x, round_ovl_chn_y);
            ret = -1;
            goto check_done;
        }
   }
#endif//hlayer 1 check done
check_done:
    lc->bLayerRectCkedFlag = 1;
    lc->mVideoframeCallback(lc->pUserData,pBuf, nDoubleLayerErrType);
    return ret;
}
...
static int __LayerQueueBuffer(LayerCtrl* l, VideoPicture* pBuf, int bValid)
{
    ...
    if (!lc->bLayerRectCkedFlag && (LayerCheckDoubleVideoRect(l, pBuf) < 0))
        loge("layer check double video cond failed, please check the layer rect setting\n");
...
static int __LayerReturnBuffer(LayerCtrl* l, VideoPicture** ppVideoPicture)
{
    LayerContext* lc;
    VPictureNode* nodePtr = NULL;
    VideoPicture* pPicture = NULL;
    int i = 0;
    ...
    logv("queue buffer num = %d", i);
    if(i > 1 && lc->pPictureListHeader != NULL)
    {
        nodePtr = lc->pPictureListHeader;
        lc->pPictureListHeader = lc->pPictureListHeader->pNext;
        pPicture = nodePtr->pPicture;
...
static LayerControlOpsT mLayerControlOps =
{
    ...
    //returnBuffer:               __LayerReturnBuffer              ,
};
...
int createG2dRotateBuf(LayerCtrl* l,VideoPicture* pBuf,int rotateDegree){
    ...
    //Since,open the lbc,G2D does not support direct rotation of 180¡ã,
    //the buffer is used to assist the rotation of 90¡ã twice.
