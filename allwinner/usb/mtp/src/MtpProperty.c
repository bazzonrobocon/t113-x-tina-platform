...
static void append(uint32_t handle, uint32_t property, uint32_t type, void *Value, struct MtpPropertyList *list)
{
	...
	switch (type) {
		...
		case MTP_TYPE_UINT16:
			...
			if (list->mLongValues[index] == NULL) {
				fprintf(stderr, "%s failed, line:%d list->mLongValues[%d] calloc failed!\n",
...
static struct MtpPropertyList *MtpPropertyListInit(uint32_t maxCount)
{
	...
	if (list == NULL) {
		fprintf(stderr, "%s failed, MtpPropertyList calloc failed!\n", __func__);
		return NULL;
	}
	...
	if (list->mObjectHandles == NULL) {
		fprintf(stderr, "%s failed, list->mObjectHandles calloc failed!\n", __func__);
		goto free_list;
	}
	list->mPropertyCodes = calloc_wrapper(maxCount, sizeof(uint16_t));
	if (list->mPropertyCodes == NULL) {
		fprintf(stderr, "%s failed, list->mPropertyCodes calloc failed!\n", __func__);
		goto free_handles;
	}
	list->mDataTypes = calloc_wrapper(maxCount, sizeof(uint16_t));
	if (list->mDataTypes == NULL) {
		fprintf(stderr, "%s failed, list->mDataTypes calloc failed!\n", __func__);
		goto free_codes;
	}
	list->mLongValues = calloc_wrapper(maxCount, sizeof(uint32_t **));
	if (list->mLongValues == NULL) {
		fprintf(stderr, "%s failed, list->mLongValues calloc failed!\n", __func__);
		goto free_types;
	}
	list->mStringValues = calloc_wrapper(maxCount, sizeof(char **));
	if (list->mStringValues == NULL) {
		fprintf(stderr, "%s failed, list->mStringValues calloc failed!\n", __func__);
		goto free_values;
	}
	list->append = append;
...
struct MtpProperty *MtpPropertyInit(MtpPropertyCode propCode, MtpDataType type, bool writeable, int defaultValue)
{
	...
	if (mProperty == NULL) {
		fprintf(stderr, "%s failed, mProperty calloc failed!\n", __func__);
		return NULL;
	}
