...
enum {
	...
	MTP_TOOLS_FUNCTION_UEVENT_CONNECT = 50,
	MTP_TOOLS_FUNCTION_UEVENT_DISCONNECT,
...
int mtp_tools_send_command(int fd, uint32_t action, uint32_t type, const char *spath, const char *dpath)
{
	...
	if (spath != NULL) {
		...
		action != MTP_TOOLS_FUNCTION_UEVENT_CONNECT &&
		action != MTP_TOOLS_FUNCTION_UEVENT_DISCONNECT) {
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		const struct option long_options[] = {
			...
			{"uevent", required_argument, NULL, 'u'},
			{"help", no_argument, NULL, 'h'},
		};
		int option_index = 0;
		int c = 0;

		c = getopt_long(argc, argv, "f:t:s:d:u:h", long_options, &option_index);
		if (c == -1)
			break;
		...
		case 'u':
			if (!optarg)
				usage();
			if (!strcmp(optarg, "connect"))
				func = MTP_TOOLS_FUNCTION_UEVENT_CONNECT;
			else if (!strcmp(optarg, "disconnect"))
				func = MTP_TOOLS_FUNCTION_UEVENT_DISCONNECT;
			break;
		default:
			usage();
			break;
	...
	if (optind < 2)
		usage();
	if ((func != MTP_TOOLS_FUNCTION_CONNECT &&
		func != MTP_TOOLS_FUNCTION_UEVENT_CONNECT &&
		func != MTP_TOOLS_FUNCTION_UEVENT_DISCONNECT)
		&& !spath) {
		printf("need spath!\n");
