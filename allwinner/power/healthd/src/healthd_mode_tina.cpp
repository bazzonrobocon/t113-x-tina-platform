...
void healthd_mode_tina_battery_update(struct BatteryProperties *props, char *node)
{
	...
	if (props->batteryPresent &&
		...
		/*low capacity handle */
		low_capacity_shutdown = batinfo_get_bat_capacity_alert_level2(node);
		if (low_capacity_shutdown >= 0 &&
			props->batteryLevel <= low_capacity_shutdown) {
			/*low capacity shutdown */
			perform_shutdown(props);
		}
		low_capacity_warning = batinfo_get_bat_capacity_alert_level1(node);
	...
	ic_temp = batinfo_get_ic_temp(node);
	if (ic_temp >= 0) {
