...
void *_softap_mode_main_loop(void *arg)
{
	...
	int wait_second = main_loop_para->wait_second;
	...
	if(wait_second <= 0) {
		wait_second = DEFAULT_SECOND;
	}
	tv.tv_sec = wait_second;
	tv.tv_usec = 0;
	if(setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) < 0) {
		WMG_ERROR("setsockopt failed");
		main_loop_para->result_cb(NULL);
		goto soket_end;
	} else {
		WMG_INFO("set sockopt timeout %d second\n",wait_second);
	}

	while(1) {
		WMG_INFO("recvfrom...\n");
		sock_ret = recvfrom(sock, buff, 511, 0, (struct sockaddr*)&clientAddr, &len);
		WMG_INFO("%s *****\n", buff);
		if(sock_ret > 0){
			WMG_INFO("recvfrom success.\n");
			break;
		} else if(errno == EWOULDBLOCK || errno == EAGAIN) {
			WMG_WARNG("*** recvfrom timeout ***\n");
			main_loop_para->result_cb(NULL);
			goto soket_end;
		}
	}
	...
	while(1) {
		sock_ret = sendto(sock, buff_confirm, sock_ret, 0, (struct sockaddr *)&clientAddr, sizeof(clientAddr));
		if(sock_ret > 0){
