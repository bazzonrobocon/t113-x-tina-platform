                        ...
                        unsigned int duration )
{
    ...
    else if( tk->fmt.iCodec == VIDEO_CODEC_FORMAT_H264 || tk->fmt.iCodec == VIDEO_CODEC_FORMAT_H265) //* do it in remux parser
    {
        if( (tk->pBuffer[0] & 0x1f) >= 24 )
            logw(  "unsupported NAL type for %s",(tk->fmt.iCodec == VIDEO_CODEC_FORMAT_H264) ? "H264" : "H265" );
        ...
        logv("========== %s i_size(%u), impl->mCurptk.pktData(%p)",
            (tk->fmt.iCodec == VIDEO_CODEC_FORMAT_H264) ? "H264" : "H265",i_size,impl->mCurptk.pktData);
    }
#if 1
    else if( tk->bAsf )
    {
...
static int ExtractMediaInfo(MediaSubsession *sub, CdxRtspStreamImplT *impl)
{
    ...
    else if( !strcmp( sub->mediumName(), "video" ) )
    {
        ...
        {
            unsigned int i_extra_vps, i_extra_sps, i_extra_pps, i_extra= 0;
            unsigned char *p_extra_vps = NULL;
            unsigned char *p_extra_sps = NULL;
            unsigned char *p_extra_pps = NULL;

            tk->fmt.iCodec = VIDEO_CODEC_FORMAT_H265;
            pMediaInfo->program[0].video[impl->nVideotrack].eCodecFormat = VIDEO_CODEC_FORMAT_H265;

            p_extra_vps = RtspParseH264ConfigStr( sub->fmtp_spropvps(),i_extra_vps );
            p_extra_sps = RtspParseH264ConfigStr( sub->fmtp_spropsps(),i_extra_sps );
            p_extra_pps = RtspParseH264ConfigStr( sub->fmtp_sproppps(),i_extra_pps );
            if(p_extra_vps == NULL || p_extra_sps == NULL || p_extra_pps == NULL ) {
               //TODO
               logw("there maybe has some error!");
            }
            else {
               i_extra = i_extra_vps + i_extra_sps + i_extra_pps;
               pMediaInfo->program[0].video[0].nCodecSpecificDataLen = i_extra;
               pMediaInfo->program[0].video[0].pCodecSpecificData = (char *)malloc( i_extra );
               unsigned int pos = 0;
               memcpy( pMediaInfo->program[0].video[0].pCodecSpecificData + pos, p_extra_vps, i_extra_vps );
               pos += i_extra_vps;
               memcpy( pMediaInfo->program[0].video[0].pCodecSpecificData + pos, p_extra_sps, i_extra_sps );
               pos += i_extra_sps;
               memcpy( pMediaInfo->program[0].video[0].pCodecSpecificData + pos, p_extra_pps, i_extra_pps );
               pos += i_extra_pps;
