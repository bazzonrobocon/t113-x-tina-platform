...
static int LayerCallbackVideoframe(void* pUser,void* param, DOUBLE_VIDEO_LAYER_ERROR_TYPE lcheck){
    TPlayer* tplayer = (TPlayer*)pUser;
    ...
    if(tplayer->mNotifier)
    {
	tplayer->nDoubleVideoLayerType = lcheck;
	switch(tplayer->nDoubleVideoLayerType)
	{
           case DOUBLE_VIDEO_LAYER_DISP_RECT_ERROR:
	   {
                tplayer->mNotifier(tplayer->mUserData,TPLAYER_NOTIFY_DISP_RECT_SET_ERROR,0,&data);
                break;
	   }

	   case DOUBLE_VIDEO_LAYER_WIDTH_AREA_ERROR:
	   {
                tplayer->mNotifier(tplayer->mUserData,TPLAYER_NOTIFY_WIDTH_AREA_SET_ERROR,0,&data);
		break;
           }
	   case DOUBLE_VIDEO_LAYER_COORDINATE_ERROR:
	   {
                tplayer->mNotifier(tplayer->mUserData,TPLAYER_NOTIFY_ODD_EVEN_COORDINATE_SET_ERROR,0,&data);
		break;
           }
	   case DOUBLE_VIDEO_LAYER_DISP_DISLOCATION_ERROR:
	   {
                tplayer->mNotifier(tplayer->mUserData,TPLAYER_NOTIFY_DISP_DISLOCATION_ERROR,0,&data);
		break;
           }
	   default:
	   {
...
#if 0
        if(pic->nDispRectErrFlag)
        {
           tplayer->mNotifier(tplayer->mUserData,TPLAYER_NOTIFY_DISP_RECT_SET_ERROR,0,&data);
        }else if(pic->nWidthAreaErrFlag)
        {
           tplayer->mNotifier(tplayer->mUserData,TPLAYER_NOTIFY_WIDTH_AREA_SET_ERROR,0,&data);
        }else if(pic->nCoordinateErrFlag)
        {
           tplayer->mNotifier(tplayer->mUserData,TPLAYER_NOTIFY_ODD_EVEN_COORDINATE_SET_ERROR,0,&data);
        }else if(pic->nDispDisLocationErrFlag)
        {
           tplayer->mNotifier(tplayer->mUserData,TPLAYER_NOTIFY_DISP_DISLOCATION_ERROR,0,&data);
        }
        else
...
int TPlayerGetAudioBalance(TPlayer* p)
{
    TP_CHECK(p);
    TP_CHECK(p->mXPlayer);
    return XPlayerControl(p->mXPlayer, XPLAYER_CMD_GET_AUDIO_BALANCE, NULL, NULL);
}
int TPlayerSetAudioBalance(TPlayer* p,int val)
{
    TP_CHECK(p);
    TP_CHECK(p->mXPlayer);
    return XPlayerControl(p->mXPlayer, XPLAYER_CMD_SET_AUDIO_BALANCE, val, NULL);
}
...
int TPlayerGetAudioTrack(TPlayer* p)
{
    TP_CHECK(p);
    TP_CHECK(p->mXPlayer);
    return XPlayerControl(p->mXPlayer, XPLAYER_CMD_GET_AUDIO_TRACK, NULL, NULL);
}
...
TPlayer* TPlayerCreate(TplayerType type){
    ...
    mPrivateData->nDoubleVideoLayerType = 0;
    ...
    if(type == CEDARX_PLAYER){
        ...
        int bUsedDeinterlace = GetConfigParamterInt("enable_deinterlace", 0);
        if (bUsedDeinterlace == 1) {
            ...
            TLOGD("not using deinterlace for playing video.");
            mPrivateData->mDeinterlace = NULL;
        }
    ...
    mPrivateData->bHoldLastPictureFlag = GetConfigParamterInt("hold_last_picture_flag", 1);
...
int TPlayerStart(TPlayer* p){
    ...
    if (p->bHoldLastPictureFlag) {
        TPlayerSetHoldLastPicture(p, 1);
    }
...
int TPlayerSetScaleDownRatio(TPlayer* p, TplayerVideoScaleDownType nHorizonScaleDown, TplayerVideoScaleDownType nVerticalScaleDown){
    ...
    return XPlayerControl(p->mXPlayer,XPLAYER_CMD_SET_SCALEDOWN_RATIO, horizonScaleDown, verticalScaleDown);
#endif
}
...
int TPlayerSetRotate(TPlayer* p, TplayerVideoRotateType rotate){
      ...
      return -1;
#else
    return XPlayerControl(p->mXPlayer, XPLAYER_CMD_SET_ROTATE_DEGREE, (int)rotate, NULL);
#endif
}
...
int TPlayerSetSubCharset(TPlayer* p, const char* strFormat)
{
	...
	return XPlayerSetSubCharset(p->mXPlayer, strFormat);
#endif
}
...
int TPlayerGetVideoDispFramerate(TPlayer* p,float* dispFramerate)
{
    ...
    TP_CHECK(p);
    TP_CHECK(p->mXPlayer);
    return XPlayerControl(p->mXPlayer, XPLAYER_CMD_GET_VIDEO_DISP_FRAME_RATE, dispFramerate, NULL);
#endif
}
...
int TPlayerSetHoldLastPicture(TPlayer* p,int bHoldFlag)
{
    ...
    TP_CHECK(p);
    TP_CHECK(p->mXPlayer);
    p->bHoldLastPictureFlag = bHoldFlag;
    return XPlayerControl(p->mXPlayer, XPLAYER_CMD_SET_HOLD_LAST_PICTURE, bHoldFlag, NULL);
#endif
}
