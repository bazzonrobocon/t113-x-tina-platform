...
bool BatteryMonitor::update(void)
{
	...
	/* present */
	mHealthdConfig->batteryPresent = batinfo_get_bat_present(mHealthdConfig->batteryNode);
	...
	/* capacity */
	mHealthdConfig->batteryCapacity = batinfo_get_bat_capacity(mHealthdConfig->batteryNode);
	...
	/* voltage_now */
	mHealthdConfig->batteryVoltage = batinfo_get_bat_voltage(mHealthdConfig->batteryNode);
	...
	/* temperture */
	mHealthdConfig->batteryTemperature = batinfo_get_bat_temp(mHealthdConfig->batteryNode);
	...
	/* current */
	mHealthdConfig->batteryCurrentNow = batinfo_get_bat_current(mHealthdConfig->batteryNode);
	...
	/* status */
	ret = batinfo_get_bat_status(mHealthdConfig->batteryNode);
	if (ret > 0) {
	...
	/* health */
	ret = batinfo_get_bat_health(mHealthdConfig->batteryNode);
	if (ret > 0) {
	...
	/* ac online */
	AcOnline = batinfo_get_ac_present(mHealthdConfig->acNode);
	props.chargerAcOnline = AcOnline;
	TLOGI("update-ac online=%d\n", AcOnline);

	/* usb online */
	UsbOnline = batinfo_get_usb_present(mHealthdConfig->usbNode);
	props.chargerUsbOnline = UsbOnline;
	TLOGI("update-usb online=%d\n", UsbOnline);

	healthd_mode_ops->battery_update(&props, mHealthdConfig->batteryNode);
...
void BatteryMonitor::init(struct healthd_config *hc, int *charger)
{
	...
	if (dir == NULL) {
		...
		while ((entry = readdir(dir))) {
			...
			if (strstr(name, "usb") != NULL) {
				memcpy(mHealthdConfig->usbNode, name, strlen(name));
				LastUsbOnline = batinfo_get_usb_present(mHealthdConfig->usbNode);
				TLOGI("Init usb_online=%d\n", LastUsbOnline);
			} else if (strstr(name, "ac") != NULL) {
				memcpy(mHealthdConfig->acNode, name, strlen(name));
				LastAcOnline = batinfo_get_ac_present(mHealthdConfig->acNode);
				TLOGI("Init ac_online=%d\n", LastAcOnline);
			} else if (strstr(name, "battery") != NULL) {
				memcpy(mHealthdConfig->batteryNode, name, strlen(name));
