...
int CdcDmaheapAllocFd(int fd,size_t len)
{
    int ret = 0;
    aw_dma_heap_allocation_data_t heap_data = {
        .len = len,
        .fd_flags = O_RDWR | O_CLOEXEC,
        };

    ret = ioctl(fd, AW_DMA_HEAP_IOCTL_ALLOC, &heap_data);
    if (ret < 0)
    {
        loge("CdcDmaheapAllocFd err, ret %d\n",ret);
        loge("CdcDmaheapAllocFd err errno=%d reason=%s",errno,strerror(errno));
        return -1;
    }
    logv("ret:%d heap_data.fd:%d",ret,heap_data.fd);
    return heap_data.fd;
}
...
int CdcIonOpen(void)
{
    ...
    logv("fd:%d",fd);
    if (fd < 0)
        loge("open /dev/dma_heap failed: %s\n",strerror(errno));
#else
