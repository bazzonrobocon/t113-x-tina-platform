...
#include "TRcommon.h"
#define GET_CAMERA_DATA_TIMEDIFF (0)
...
int VportRequestbuf(void *hdl, int count)
{
	...
	for (i = 0; i < handle->buf_num; i++) {
		...
		case V4L2_MEMORY_DMABUF:
			if (handle->driveType == other_drive) {
				buf.m.fd = SunxiMemGetBufferFd(handle->pMemops, handle->buffers[i].pVirAddr[0]);
			} else {
				for (planes = 0; planes < handle->nplanes; planes++) {
					buf.m.planes[planes].m.fd = SunxiMemGetBufferFd(handle->pMemops, handle->buffers[i].pVirAddr[planes]);
				}
			}
			break;
...
#if GET_CAMERA_DATA_TIMEDIFF
        long long get_camera_data_start = GetNowMs();
#endif
	ret = ioctl(handle->fd, VIDIOC_DQBUF, &buf);
#if GET_CAMERA_DATA_TIMEDIFF
        long long get_camera_data_end = GetNowMs();
        printf("[%s:%d], get camera data timediff:%lldms\n", __func__, __LINE__, (get_camera_data_end -get_camera_data_start));
#endif
	if (ret != 0) {
...
int VportEnqueue(void *hdl, unsigned int index)
{
	...
	case V4L2_MEMORY_DMABUF:
		if (handle->driveType == other_drive) {
			buf.m.fd = SunxiMemGetBufferFd(handle->pMemops, handle->buffers[buf.index].pVirAddr[0]);
		} else {
			for (planes = 0; planes < handle->nplanes; planes++) {
				buf.m.planes[planes].m.fd = SunxiMemGetBufferFd(handle->pMemops, handle->buffers[buf.index].pVirAddr[planes]);
			}
		}
		break;
	default:
