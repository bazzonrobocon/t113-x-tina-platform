...
#include "TRlog.h"
#define WRITE_RECORDER_FILE_TIMEDIFF (0)
...
int __CdxWrite(CdxWriterT *writer, void *buf, int size)
{
	...
	if (recoder_writer->file_mode == FD_FILE_MODE) {
		...
		//TRlog(TR_LOG_MUXER, "detect   dev/mmcblk0!!!!!!!!\n");
#endif
#if WRITE_RECORDER_FILE_TIMEDIFF
        long long write_recorder_data_start = GetNowMs();
#endif
		ret = fwrite(pbuf, 1, size, recoder_writer->fp_file);
#if WRITE_RECORDER_FILE_TIMEDIFF
        long long write_recorder_data_end = GetNowMs();
	if((write_recorder_data_end - write_recorder_data_start) >100)
	{
            printf("[%s:%d], write recorder data timediff:%lldms\n", __func__, __LINE__, (write_recorder_data_end-write_recorder_data_start));
	}
#endif
		if (size && ret != size) {
