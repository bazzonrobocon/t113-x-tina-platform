...
#ifdef USE_MTPTOOLS_CONNECT
	if (command->action == MTP_TOOLS_FUNCTION_UEVENT_CONNECT) {
		printf("mtp tools event recv connected state\n");
		mtp_set_usb_state(USB_STATE_CONNECTED);
		return;
	} else if (command->action == MTP_TOOLS_FUNCTION_UEVENT_DISCONNECT) {
		printf("mtp tools event recv disconnected state\n");
		mtp_set_usb_state(USB_STATE_DISCONNECTED);
		return;
	}
#endif
