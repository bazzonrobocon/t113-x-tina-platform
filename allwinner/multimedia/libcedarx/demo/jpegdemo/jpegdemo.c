...
static void JpegSaveNormalPicture(struct ScMemOpsS* memops, VideoPicture *videoPicture, char *pOutUri)
{
    ...
    CdcMemFlushCache(memops, videoPicture->pData0, videoPicture->nWidth*videoPicture->nHeight * 3/2);
#if SAVE_RGB
    ...
    switch (ret)
    {
        ...
        case VDECODE_RESULT_NO_FRAME_BUFFER:
        {
            ...
            if (ret>= 0)
            {
                ...
                {
                    JpegSaveNormalPicture(memops, videoPicture, pOutUri);
                }
                sync();
            }
            else
