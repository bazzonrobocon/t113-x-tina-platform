...
//    int len = strlen(value);
    ...
    switch(arg)
    {
        ...
        case INPUT:
            ...
            {
                sscanf(argv[pos+i+1], "%2048s", params->pInputFile[i]);
        ...
        case SAVE_FRAME_FILE:
            for(i = 0; i <length -1 ; i++)
            {
                sscanf(argv[pos+i+1], "%2048s", params->pOutputFile[i]);
        ...
        case SAVE_SHA_FILE:
            for(i = 0; i <length -1 ; i++)
            {
                sscanf(argv[pos+i+1], "%2048s", params->pSaveShaFile[i]);
        ...
        case COMPARE_SHA:
            for(i = 0; i <length -1 ; i++)
            {
                sscanf(argv[pos+i+1], "%2048s", params->pCompareShaFile[i]);
