...
static int initDecoder(const char* url, DecDemo *Decoder)
{
    ...
    /*
   if((Decoder->nHeight%16)!=0)
   {
       Decoder->nHeight=(Decoder->nHeight/16+1)*16;
   }
   if((Decoder->nWidth%16)!=0)
   {
       Decoder->nWidth=(Decoder->nWidth/16+1)*16;
   }
   */
...
void *parserThreadFunc(void* param)
{
    ...
    while (nRet == 0)
    {
        ...
        pDec->nFinishNum ++;
        logd("parserThreadFunc: nFinishNum = %d\n",pDec->nFinishNum);
    ...
    /*
    //TODO：Adding this logic will cause the screen to be blurred, and the reason will be analyzed
    dequeueBuffer:
        pBuffer = dequeneInputBuffer(pDec->codecCtx);
        if(pBuffer==NULL)
        {
        ...
        else
        {
            ...
            queneInputBuffer(pDec->codecCtx, pBuffer);
            logd("#####set eos flag %ld!",pBuffer->nFlags);
        }
    */
    pthread_rwlock_unlock(&pDec->thread.rwrock);
parser_exit:
