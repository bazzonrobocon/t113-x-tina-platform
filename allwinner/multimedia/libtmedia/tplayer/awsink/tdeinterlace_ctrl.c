...
static int getVeOffset()
{
    ...
    */
    if (strcmp(vd_platform, "r40") == 0) {//0x1701
        ...
        logw("the platform[%s] might be built in tina5.0. VE offset will be set to VE_OFFSET_ADDR:%x as config", vd_platform, ve_offset);
    }
    return ve_offset;
}
...
#ifdef DI_V1XX
static void dump_div1xx_arg(struct di_process_fb_arg* di)
{
    printf("*************************************************\n");
    printf("di->is_interlace = %d  di->field_order = %d\n", di->is_interlace,di->field_order);
    printf("di->pixel_format = 0x%x\n", di->pixel_format);
    printf("di->size.width = %d di->size.height = %d \n", di->size.width, di->size.height);
    printf("di->output_mode = %d di->di_mode = %d\n", di->output_mode, di->di_mode);
    printf("di->in_fb0.dma_buf_fd = %d\n", di->in_fb0.dma_buf_fd);
    printf("di->in_fb0.buf.addr.y_addr = 0x%llx\n", di->in_fb0.buf.addr.y_addr);
    printf("di->in_fb0.buf.addr.cb_addr = 0x%llx\n", di->in_fb0.buf.addr.cb_addr);
    printf("di->in_fb0.size.width = %d\n", di->in_fb0.size.width);
    printf("di->in_fb0.size.height = %d\n", di->in_fb0.size.height);

    printf("di->in_fb1.dma_buf_fd = %d\n", di->in_fb1.dma_buf_fd);
    printf("di->in_fb1.buf.addr.y_addr = 0x%llx\n", di->in_fb1.buf.addr.y_addr);
    printf("di->in_fb1.buf.addr.cb_addr = 0x%llx\n", di->in_fb1.buf.addr.cb_addr);
    printf("di->in_fb1.size.width = %d\n", di->in_fb1.size.width);
    printf("di->in_fb1.size.height = %d\n", di->in_fb1.size.height);

    printf("di->out_fb0.dma_buf_fd = %d\n", di->out_fb0.dma_buf_fd);
    printf("di->out_fb0.buf.addr.y_addr = 0x%llx\n", di->out_fb0.buf.addr.y_addr);
    printf("di->out_fb0>.buf.addr.cb_addr = 0x%llx\n", di->out_fb0.buf.addr.cb_addr);
    printf("di->out_fb0>.size.width = %d\n", di->out_fb0.size.width);
    printf("di->out_fb0>.size.height = %d\n", di->out_fb0.size.height);
    printf("*********************OVER************************\n");
}

static int __Div1xx_Process(Deinterlace* di,
                    VideoPicture *pPrePicture,
                    VideoPicture *pCurPicture,
                    VideoPicture *pOutPicture,
                    int nField)
{
    ...
    memset(&size, 0, sizeof(struct di_size));
    size.width = pCurPicture->nWidth;
    size.height = pCurPicture->nHeight;

    memset(&arg, 0, sizeof(struct di_process_fb_arg));

    if (pOutPicture->ePixelFormat == PIXEL_FORMAT_NV12)
    {
        arg.pixel_format = DRM_FORMAT_NV12;
    }
    else if (pOutPicture->ePixelFormat == PIXEL_FORMAT_NV21)
    {
        arg.pixel_format = DRM_FORMAT_NV21;
    }
    else
    {
        loge("the inputPixelFormat is not support : %d",pOutPicture->ePixelFormat);
    ...
    arg.size                         = size;
    arg.is_interlace                 = 1;
    arg.field_order                  = pOutPicture->bTopFieldFirst ?
                                        DI_TOP_FIELD_FIRST : DI_BOTTOM_FIELD_FIRST;
    arg.output_mode                  = DI_OUT_1FRAME;
    arg.di_mode                      = DI_INTP_MODE_MOTION;

    arg.in_fb0.dma_buf_fd            = pPrePicture->nBufFd;
    arg.in_fb0.size                  = size;
    arg.in_fb0.buf.offset.y_offset   = 0;
    arg.in_fb0.buf.offset.cb_offset  = size.width * size.height;

    arg.in_fb1.dma_buf_fd            = pCurPicture->nBufFd;
    arg.in_fb1.size                  = size;
    arg.in_fb1.buf.offset.y_offset   = 0;
    arg.in_fb1.buf.offset.cb_offset  = size.width * size.height;

    arg.out_fb0.dma_buf_fd           = pOutPicture->nBufFd;
    arg.out_fb0.size                 = size;
    arg.out_fb0.buf.offset.y_offset  = 0;
    arg.out_fb0.buf.offset.cb_offset = size.width * size.height;

    //dump_div1xx_arg(&arg);
    //long long start_time = CdxGetNowUs();
    if (ioctl(dc->fd, DI_IOC_PROCESS_FB, &arg) < 0)
    {
        loge("DI_IOC_PROCESS_FB failed\n");
        return -1;
    }
    //long long end_time = CdxGetNowUs();
    //long long time_diff = end_time - start_time;
    //printf("[THH][%s %d]check ioctl of DI_IOC_PROCESS_FB timecost:%lld\n",__func__, __LINE__, time_diff);
    ...
    if (dc->saveCount < 300)
    {
        char outPath[128];
        memset(outPath, 0, sizeof(outPath));
        sprintf(outPath,"/mnt/SDCARD/di/out_%d_%d.yuv", size.width, size.height);
        FILE* outFp = fopen(outPath,"ab+");
        if(outFp == NULL){
            logd("open %s fail",outPath);
            return -1;
        }else{
            fwrite(pOutPicture->pData0, size.width * size.height, 1, outFp);
            fwrite(pOutPicture->pData1, (size.width * size.height) / 2, 1, outFp);
            fclose(outFp);
        }
        dc->saveCount++;
    }
