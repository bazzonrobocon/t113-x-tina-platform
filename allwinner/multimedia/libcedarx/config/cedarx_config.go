...
var saturn_T_cflags = []string {
    "-DCONF_ANDROID_SUB_VER=1",
    "-DCONF_KERNEL_VERSION_5_15",
    "-DCONF_SIMPLE_ION",
    ...
    "-DCONF_DI_SINGEL_FILE",
}

var ceres_U_cflags = []string {
    "-DCONF_ANDROID_SUB_VER=1",
    "-DCONF_KERNEL_VERSION_5_15",
    "-DCONF_SIMPLE_ION",
    "-DCONF_GPU_IMG_USE_COMMON_STRUCT",//After ceres, MALI GPU and IMG GPU use the same structure for users
...
var cdxCfgTable []cdxCfgTableType = []cdxCfgTableType {
    cdxCfgTableType {
        34,
        []string {"-DCONF_U_AND_NEWER", "-DCONF_ANDROID_MAJOR_VER=14"},
        []cdxCfgType {
            cdxCfgType {"ceres",   ceres_U_cflags},
            cdxCfgType {"saturn",  saturn_T_cflags},
        },
    },
    ...
    cdxCfgTableType {
        ...
        []cdxCfgType {
            ...
            cdxCfgType {"saturn",  saturn_T_cflags},
            cdxCfgType {"ceres",   ceres_T_cflags},
            cdxCfgType {"epic",    ceres_T_cflags},
...
func globalDefaults(ctx android.BaseContext) ([]string) {
    ...
    afbc := ctx.Config().VendorConfig("aw_media").String("afbc_mode")
    if len(afbc) > 0 && afbc != "0"{
        fmt.Printf("cedarx-config: media afbc mode: [%s]\n", afbc)
        cppflags = append(cppflags, "-DCONF_AFBC_ENABLE=" + afbc)
    } else {
        fmt.Printf("[%s] did not config afbc mode!\n", board)
    }
    return cppflags
}
