...
int handleControlRequest(const struct usb_ctrlrequest *setup, struct MtpDevHandle *handle)
{
	...
	if ((type & USB_TYPE_MASK) == USB_TYPE_CLASS && index == 0 && value == 0) {
		DLOG("Usb setup code: 0x%x\n", code);
		switch(code) {
		case MTP_REQ_RESET:
		case MTP_REQ_CANCEL:
			errno = ECANCELED;
			return 0; //Android MtpFfsHandle return -1
			//    break;
	...
	while (num_events < min_events) {
		...
		if (mPollFds[1].revents & POLLIN) {
			...
			for (unsigned j = 0; j < (unsigned int)(this_events); j++) {
				if (events[j].res < 0) {
					errno = -events[j].res;
					fprintf(stderr, "Mtp got error event at %d and %d tatal\n", j, buf->actual);
					error = errno;
				}
				ret += events[j].res;
			}
					...
					struct MtpDevHandle *handle)
{
	...
	while (file_length > 0 || has_write) {
		...
		if (error) {
			DLOG("iobufSubmit failed, return -1\n");
			return -1;
		}
		...
		if (file_length > 0) {
			...
			while (num_events < short_i) {
				...
				if (event_ret == -1) {
					DLOG("event_ret=%d, errno=%d, num_events=%d, actual=%d", event_ret, errno, num_events, ctrl->mIobuf[i].actual);
			...
			if (short_packet) {
				DLOG("run cancel cancelEvents, short_packet is true");
			...
			if (write_error) {
				DLOG("cancelEvents failed, write_error is true\n");
