...
{
	char path[128];
	sprintf(path, "%s%s%s", POWER_SUPPLY_SYSFS_PATH, node, "/present");
	return getBooleanField(path);
}
...
int batinfo_get_bat_status(char *node)
{
	...
	sprintf(path, "%s%s%s", POWER_SUPPLY_SYSFS_PATH, node, "/status");
	if(readFromFile(path, buf, sizeof(buf)) > 0) {
...
int batinfo_get_bat_health(char *node)
{
	...
	memset(buf, 0, sizeof(buf));
	sprintf(path, "%s%s%s", POWER_SUPPLY_SYSFS_PATH, node, "/health");
	if(readFromFile(path, buf, sizeof(buf)) > 0) {
...
{
	char path[128];
	sprintf(path, "%s%s%s", POWER_SUPPLY_SYSFS_PATH, node, "/capacity");
...
{
	char path[128];
	sprintf(path, "%s%s%s", POWER_SUPPLY_SYSFS_PATH, node, "/capacity_alert_max");
	return getIntField(path);
}
...
{
	char path[128];
	sprintf(path, "%s%s%s", POWER_SUPPLY_SYSFS_PATH, node, "/capacity_alert_min");
	return getIntField(path);
}
...
{
	char path[128];
	sprintf(path, "%s%s%s", POWER_SUPPLY_SYSFS_PATH, node, "/voltage_now");
	return getIntField(path);
}
...
{
	char path[128];
	sprintf(path, "%s%s%s", POWER_SUPPLY_SYSFS_PATH, node, "/current_now");
	return getIntField(path);
}
...
{
	char path[128];
	sprintf(path, "%s%s%s", POWER_SUPPLY_SYSFS_PATH, node, "/temp");
	return getIntField(path);
}
...
{
	char path[128];
	sprintf(path, "%s%s%s", POWER_SUPPLY_SYSFS_PATH, node, "/temp_ambient");
	return getIntField(path);
}
		...
		char *acNode, char *batteryNode)
{
	if(props == NULL)
		return -1;
	props->chargerAcOnline = batinfo_get_ac_present(acNode);
	props->chargerUsbOnline = batinfo_get_usb_present(usbNode);
	props->batteryPresent = batinfo_get_bat_present(batteryNode);
	props->batteryStatus = batinfo_get_bat_status(batteryNode);
	props->batteryHealth = batinfo_get_bat_health(batteryNode);
	props->batteryLevel = batinfo_get_bat_capacity(batteryNode);
	props->batteryVoltage = batinfo_get_bat_voltage(batteryNode);
	props->batteryCurrentNow = batinfo_get_bat_current(batteryNode);
	props->batteryTemperature = batinfo_get_bat_temp(batteryNode);
	return 0;
}
