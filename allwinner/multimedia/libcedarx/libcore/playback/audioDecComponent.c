...
#if TINA_LINUX_SUPPORT
        if(pStreamInfo[i].eCodecFormat == AUDIO_CODEC_FORMAT_APE)
        {
            p->pStreamInfoArr[i].nAudioStreamBufferSize = 1*1024*1024;
            p->pStreamInfoArr[i].nAudioPcmBufferSize = 512*1024;
        }else
        {
            p->pStreamInfoArr[i].nAudioStreamBufferSize = GetConfigParamterInt("alib_audio_bitstream_buffer_size", 4*1024)*1024;
            p->pStreamInfoArr[i].nAudioPcmBufferSize = GetConfigParamterInt("alib_audio_pcm_buffer_size", 512)*1024;
        }
#endif
    }
