...
void *OutputThreadFunc(void* param)
{
    ...
    while(1)
    {
        ...
        if(pBuffer != NULL && pBuffer->pBuffer != NULL)
        {
            ...
            loge("nGetFrameCount=%d,nFinishNum=%d, pBuffer->nFilledLen=%d\n",nGetFrameCount,pVideoEncoder->nFinishNum, (int)pBuffer->nFilledLen);
...
void *InputThreadFunc(void* param)
{
    ...
    loge("nRet:%d, readSize::%d", nRet, readSize);
    while (1)
    {
        loge("enter while loop");
        usleep(30*1000);
        ...
        if (pBuffer == NULL) {
            loge("input pBuffer = NULL,continue");
            continue;
        }
