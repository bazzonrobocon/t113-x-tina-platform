...
/* THH:87f3f3b and 81b714e */
#if 0
    /* *****************************************************************
     * * code below might cause a static code check error that
...
#else
    CdxHttpHeaderFieldsT* header;
    header = (CdxHttpHeaderFieldsT*)malloc(sizeof(CdxHttpHeaderFieldsT));
    if (!header) {
        CDX_LOGE("malloc header failed!");
        datasource->extraData = NULL;
    } else {
        memset(header, 0, sizeof(CdxHttpHeaderFieldsT));
        header->num = 7;
        memcpy(header->pHttpHeader, hdr, sizeof(hdr));
        datasource->extraData = (void*)header;
    }
#endif
