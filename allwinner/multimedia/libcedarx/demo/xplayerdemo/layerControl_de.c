...
#if ((defined CONF_KERNEL_VERSION_5_10) || \
     (defined CONF_KERNEL_VERSION_5_15))
...
static int setLayerBuffer(LayerContext* lc)
{
    ...
    for(i=0; i<lc->nGpuBufferCount; i++)
    {
        ...
        //set real buf fd to picture, or the ve will fail to map
        lc->mBufferInfo[i].pPicture.nBufFd       = CdxIonGetBufferFd(pVirBuf);
        lc->mBufferInfo[i].pPicture.ePixelFormat = lc->eDisplayPixelFormat;

        logd("=== init id:%d, nBufFd: %d, pVirBuf: %p, pPhyBuf :%p", i, lc->mBufferInfo[i].pPicture.nBufFd, pVirBuf, pPhyBuf);
    }
...
static int SetLayerParam(LayerContext* lc, VideoPicture* pPicture)
{
    ...
    switch(lc->eDisplayPixelFormat)
    {
        ...
        case PIXEL_FORMAT_YV12:
            config.info.fb.format = DISP_FORMAT_YUV420_P;//need to fix, DISP_FORMAT_YUV420_P means Y-U-V
            config.info.fb.addr[0]  = (unsigned long )
                                        CdxIonVir2Phy(pPicture->pData0);
            config.info.fb.addr[1]  = (unsigned long )
                                        CdxIonVir2Phy(pPicture->pData2);
...
static int __LayerSetDisplayPixelFormat(LayerCtrl* l, enum EPIXELFORMAT ePixelFormat)
{
    ...
    if(ePixelFormat == PIXEL_FORMAT_NV12 ||
       ...
       ePixelFormat == PIXEL_FORMAT_YUV_PLANER_420) //* add new pixel formats supported by gpu here.
    {
...
#ifndef __ANDROID__
    //we clear screen first in case launcher is running on the top
    int fd = -1;
    if ((fd = open("/dev/fb0", O_RDWR)) == -1) {
        loge("open /dev/fb0 fail.Fail to clear screen!");
        return;
    }
...
static VideoPicture* __LayerGetBufferOwnedByGpu(LayerCtrl* l)
{
    ...
    for(i = 0; i<lc->nGpuBufferCount; i++)
    {
        if(lc->mBufferInfo[i].nUsedFlag == 1)
        {
            lc->mBufferInfo[i].nUsedFlag = 0;
...
LayerCtrl* LayerCreate_DE()
{
    ...
    if (lc->nScreenWidth <= 0 || lc->nScreenHeight <= 0)
    {
        if(lc->fdDisplay >= 0)
            close(lc->fdDisplay);
        lc->fdDisplay = -1;
        free(lc);
        loge("get screen width or height failed, w:%d, h:%d", lc->nScreenWidth, lc->nScreenHeight);
        return NULL;
    }
    CdxIonOpen();
