...
#endif
#ifdef SUPPORT_LINKD
	printf("wifi -p (softap/ble/xconfig/soundwave) [Time]\n"
		"\t: softap/ble/xconfig/soundwave distribution network\n"
		"\t: Time: maximum time 600 seconds\n"
		"\t:       if not set, it will be configured as 180 seconds\n");
#endif
	printf("wifi -f\n"
		"\t: close wifimanager\n"
		"wifi -h\n"
		...
		switch (ch) {
			...
			case 'p':
				if(argc == 3) {
					...
					sprintf(snd_buf, "p %s %s",argv[2], argv[3]);
				} else {
					print_help();
					return -1;
				}
