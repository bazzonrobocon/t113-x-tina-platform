...
int SubtitleUtilsFillBitmapSubtitleToParcel(Parcel* parcelDst,
                                SubtitleItem* pSubItem, int nSubtitleID, int startPos)
{
    //* write the size of the text sample, we only process PIXEL_FORMAT_RGBA_8888 format currently.
    int nBufSize = pSubItem->nBitmapWidth*pSubItem->nBitmapHeight*4;

    /*
     * binder limit oneway binder parcel size under 508KB, so if parcel data is too large
     * we need to send it twice, and we only send argb data into first parcel
     */
    if(nBufSize - startPos > SUBTITLE_MAX_PACKET_DATA_SZIE) {
        nBufSize = SUBTITLE_MAX_PACKET_DATA_SZIE;
        //* write the argb buffer as an int32_t array.
        parcelDst->writeInt32(nBufSize/4);
        parcelDst->write(pSubItem->pBitmapData + startPos, nBufSize);
        //  write subtitle id which hold these rgb data.
        parcelDst->writeInt32(nSubtitleID);
        return nBufSize;
    }
