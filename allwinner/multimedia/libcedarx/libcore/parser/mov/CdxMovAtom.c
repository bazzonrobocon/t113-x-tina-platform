...
const int priv_mpeg4audio_sample_rates[16] = {
    96000, 88200, 64000, 48000, 44100, 32000,
    24000, 22050, 16000, 12000, 11025, 8000, 7350
};

const int mpeg4audio_channels[8] = {
    0, 1, 2, 3, 4, 5, 6, 8
};
...
static int read_bit_from_buffer(unsigned char *buffer, int start_bit, int num_bits)
{
    int result = 0;
    ...
    for (int i = 0; i < num_bits; i++)
    {
        ...
        int bit_offset = 7 - ((start_bit + i) % 8);

        unsigned char byte = buffer[byte_index];
        int bit_value = (byte >> bit_offset) & 1;

        result |= bit_value << (num_bits - i -1);
    }

    return result;
}

/* form ISO/IEC 14496-3  1.6.2.1 */
static int mp4ParserAudioExtraData(MOVStreamContext *st)
{
    int start_bit = 0;
    int object_type = 0;
    int sampleing_frequency_index = 0;
    int channel_configuration = 0;
    unsigned char *extradata = (unsigned char *)st->codec.extradata;

    object_type = read_bit_from_buffer(extradata, start_bit, 5);
    start_bit += 5;
    if (object_type == 31)
    {
        int audio_object_type_Ext = read_bit_from_buffer(extradata, start_bit, 6);
        start_bit += 6;
        object_type = 32 + audio_object_type_Ext;
    }

    sampleing_frequency_index = read_bit_from_buffer(extradata, start_bit, 4);
    start_bit += 4;
    if (sampleing_frequency_index == 0x0F)
    {
        st->codec.sampleRate = read_bit_from_buffer(extradata, start_bit, 24);
        start_bit += 24;
    }
    else
    {
        st->codec.sampleRate = priv_mpeg4audio_sample_rates[sampleing_frequency_index];
    }

    channel_configuration = read_bit_from_buffer(extradata, start_bit, 4);
    start_bit += 4;
    st->codec.channels = mpeg4audio_channels[channel_configuration];
...
#define MP4DecSpecificDescrTag          0x05
    ...
    if (tag == MP4DecConfigDescrTag)
    {
        ...
        if (tag == MP4DecSpecificDescrTag)
        {
            ...
            /*
                some mp4 file we parser stsd atom get audio channel is stero,
                but in fact the audio channel is mono. so we should parser aduio extra data,
                get the audio actual paramters.
            */
            if (st->codec.codecType == CODEC_TYPE_AUDIO)
            {
                mp4ParserAudioExtraData(st);
            }
...
#define FFABS(a) ((a) >= 0 ? (a) : (-(a)))
    ...
    if(a.size < entries)
    {
        CDX_LOGW("entries = %u, a.size = %u, check...", entries, a.size);
        return -1;
    }
...
static int _MovParseStsc(MOVContext *c,MOV_atom_t a)
{
    ...
    if(a.size < entries)
    {
        CDX_LOGW("entries = %d, a.size = %u, check...", entries, a.size);
        return -1;
    }
