...
#define RECRODER_PCM_READ_TIMEDIFF (0)
...
#if RECRODER_PCM_READ_TIMEDIFF
        long long recorder_pcm_read_start = GetNowMs();
#endif
		ret = pcm_read(hdl->pcm, tmpBuffer, SIZE_PER_READ);
#if RECRODER_PCM_READ_TIMEDIFF
        long long recorder_pcm_read_end = GetNowMs();
        printf("[%s:%d], recorder pcm data read timediff:%lldms\n", __func__, __LINE__, (recorder_pcm_read_end- recorder_pcm_read_start));
#endif
