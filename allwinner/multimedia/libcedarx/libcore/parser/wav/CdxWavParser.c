...
static int WavInit(CdxParserT* parameter)
{
                ...
                case fmt:
                    {
                        ...
                        if (impl->WavFormat.wFormatag == WAVE_FORMAT_EXTENSIBLE)
                        {
                            cdx_uint16 realFormat = 0;
                            realFormat = (readBuf[24]&0xff) | (((cdx_int32)readBuf[25]&0xff) << 8);
                            logd("realFormat:%x %x %x size:%d", realFormat, (readBuf[24]&0xff), (((cdx_int32)readBuf[25]&0xff) << 8), WAVObject.Dsize);
                            if (realFormat != 0)
                                impl->WavFormat.wRealFormat = realFormat;
                        }
    ...
    CDX_LOGD("get the FileSize by calling CdxStreamSize: %lld \n", impl->dFileSize);
...
static cdx_int32 __WavParserGetMediaInfo(CdxParserT *parser, CdxMediaInfoT *mediaInfo)
{
    ...
    if (impl->WavFormat.wRealFormat == 0)
        cdxProgram->audio[0].eSubCodecFormat = impl->WavFormat.wFormatag | impl->nBigEndian;
    else
        cdxProgram->audio[0].eSubCodecFormat = impl->WavFormat.wRealFormat;
