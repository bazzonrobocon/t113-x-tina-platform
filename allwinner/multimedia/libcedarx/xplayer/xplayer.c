...
static int initializePlayer(XPlayer* p)
{
    ...
    if(mi->pVideoStreamInfo != NULL)
    {
        ...
        if(
#if TINA_LINUX_SUPPORT
           (mPriData->mWidthScaledownRatio == 0) && (mPriData->mHeightScaledownRatio == 0) &&
#endif
           (getScaledownFlag(mi,mPriData->bSetScaleDown)==1))/* 1 means need to scale down*/
        {
...
#if TINA_LINUX_SUPPORT
        nRotation = mPriData->mVideoRotateDegree;
#endif
        if(nRotation == 0)
            nRotateDegree = 0;
...
#if TINA_LINUX_SUPPORT
        //*  set the lbc mode */
        ret = PlayerControl(mPriData->mPlayer, PLAYER_CMD_SET_VIDEO_LBC_LOSSY_MOD, &mPriData->mVideoLbcMode);
        if(ret != 0)
        {
            logw("PlayerControl PLAYER_CMD_SET_VIDEO_LBC_LOSSY_MOD fail, ret = %d",ret);
        }
#endif
...
int XPlayerControl(XPlayer* p, int cmd, void* param1, void* param2)
{
        ...
        if(mPriData->mPlayer!=NULL)
        {
           switch(cmd)
           {
              ...
              case XPLAYER_CMD_GET_AUDIO_TRACK:
                  return PlayerControl(mPriData->mPlayer, PLAYER_CMD_GET_AUDIO_TRACK, NULL);

              case XPLAYER_CMD_GET_AUDIO_BALANCE:
                  return PlayerGetAudioBalance(mPriData->mPlayer);

              case XPLAYER_CMD_SET_AUDIO_BALANCE:
                  //* get audio balance, return 1 means left channel, 2 means right channel, 3 means stereo.
                  return PlayerSetAudioBalance(mPriData->mPlayer,(int)param1);
#endif
#ifndef ONLY_ENABLE_AUDIO
              case XPLAYER_CMD_GET_VIDEO_DISP_FRAME_RATE:
                  return PlayerControl(mPriData->mPlayer, PLAYER_CMD_GET_VIDEO_DISP_FRAME_RATE, param1);

              case XPLAYER_CMD_SET_HOLD_LAST_PICTURE:
                  return PlayerSetHoldLastPicture(mPriData->mPlayer,(int)param1);

              case XPLAYER_CMD_SET_ROTATE_DEGREE:
                  mPriData->mVideoRotateDegree = (int)param1;
                  return 0;

              case XPLAYER_CMD_SET_SCALEDOWN_RATIO:
                  if(mPriData->mStatus==XPLAYER_STATUS_IDLE
                     || mPriData->mStatus==XPLAYER_STATUS_INITIALIZED
                     || mPriData->mStatus==XPLAYER_STATUS_PREPARING){
                     mPriData->mWidthScaledownRatio = (int)param1;
                     mPriData->mHeightScaledownRatio = (int)param2;
                     return 0;
                  }else
                  {
                     return -1;
