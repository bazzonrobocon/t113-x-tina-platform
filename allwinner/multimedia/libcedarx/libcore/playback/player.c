...
int PlayerStart(Player* pl,int bSeekClosest,int nSeekTimeMs)
{
    ...
    else
    {
        ...
        if ((p->pAudioDecComp == NULL) || p->pAudioRender == NULL) {
#endif
...
int PlayerControl(Player* pl, int cmd, void* param)
{
    ...
    switch(cmd)
    {
       case PLAYER_CMD_GET_AUDIO_TRACK:
           return p->nAudioStreamSelected;

       case PLAYER_CMD_GET_VIDEO_DISP_FRAME_RATE:
           if(p->pVideoRender != NULL)
           {
              return VideoRenderCompGetDispFrameRate(p->pVideoRender, (float*)param);
           }else
           {
              return -1;
           }
       case PLAYER_CMD_SET_VIDEO_LBC_LOSSY_MOD:
           if(p->eStatus != PLAYER_STATUS_STOPPED)
           {
              loge("player not in stopped status, can not config video lbc mode.");
              return -1;
           }
           p->sVideoConfig.nLbcLossyComMod = *((int*)param);
           return CDX_SUCCESS;
       default:
           break;
    }
