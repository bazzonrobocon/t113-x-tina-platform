	...
	if(!strncmp(p, "p2p", 3)) {
...
#endif
		wmg_status_t ret = WMG_STATUS_FAIL;
		char scan_res_char[SCAN_RES_CHAR_LEN] = {0};
		...
		if (ret == WMG_STATUS_SUCCESS) {
			...
			for (i = 0; i < get_scan_results_num; i++) {
				memset(scan_res_char, 0, SCAN_RES_CHAR_LEN);
...
static int cmd_handle_cM(char *cMp)
{
	...
	if((strlen(psk_buf) >= 5) && (cn_para.sec != WIFI_SEC_NONE)) {
		cn_para.password = psk_buf;
	} else {
		WMG_WARNG("The input password is %s, but the encryption method scanned is WIFI_SEC_NONE, so we use psk(NULL) to connect\n",psk_buf);
	}
	cn_para.fast_connect = 0;
	if(ssid_connect) {
		WMG_INFO("connect to sta(%s) with pask(%s)\n", ssid_bssid_buf, (cn_para.sec != WIFI_SEC_NONE) ? psk_buf : "NULL");
...
{
	int ret = -1;
	int i, bss_num = 0, get_scan_results_flag = 0, get_scan_results_num = 0, wait_time = 0;
	char ssid_buf[SSID_MAX_LEN + 1] = {0};
	char psk_buf[PSK_MAX_LEN + 1] = {0};
	...
	if(!strncmp(p, "ble", 3)) {
		if(strlen(p) > 3) {
			wait_time = atoi(p + 4);
			if(wait_time < 0 || wait_time > 600) {
				WMG_WARNG("wait_time: %d is greater than 600 or less than 0, we set it to 0\n", wait_time);
				wait_time = 0;
			}
		}
		WMG_INFO("ble config net: wait_time(%d)\n", wait_time);
		erro_code = wifi_linkd_protocol(WMG_LINKD_MODE_BLE, NULL, wait_time, &linkd_result);
		...
		WMG_INFO("softap config net: wait_time(%d)\n", wait_time);
		wifi_ap_config_t ap_config;
		...
		erro_code = wifi_linkd_protocol(WMG_LINKD_MODE_SOFTAP, &ap_config, wait_time, &linkd_result);
		if(erro_code != WMG_STATUS_SUCCESS){
		...
		WMG_INFO("xconfig config net: wait_time(%d)\n", wait_time);
		erro_code = wifi_linkd_protocol(WMG_LINKD_MODE_XCONFIG, NULL, wait_time, &linkd_result);
		if(erro_code != WMG_STATUS_SUCCESS){
		...
		WMG_INFO("soundwave config net: wait_time(%d)\n", wait_time);
		erro_code = wifi_linkd_protocol(WMG_LINKD_MODE_SOUNDWAVE, NULL, wait_time, &linkd_result);
		if(erro_code != WMG_STATUS_SUCCESS){
