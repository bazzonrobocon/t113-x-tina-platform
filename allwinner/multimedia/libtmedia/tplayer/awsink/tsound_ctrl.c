...
#if AUDIO_WRITE_TIMEDIFF
        long long pcm_write_start = TPlayerGetNowMs();
#endif
        res = snd_pcm_writei(sc->alsa_handler, pData, num_frames);
#if AUDIO_WRITE_TIMEDIFF
        long long pcm_write_end = TPlayerGetNowMs();
	printf("[%s:%d], pcm write timediff:%lldms\n", __func__, __LINE__, (pcm_write_end-pcm_write_start));
#endif
