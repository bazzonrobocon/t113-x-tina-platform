...
_start:
    ...
    if(guess_frame != -1)
    {
        ...
        else if(os->stream->codec->codec_type == AVMEDIA_TYPE_AUDIO)
        {
            ...
            opus_pos = ogg->data_offset + practicalSize * (guess_frame + os->stream->codec->sample_rate) / ogg->total_frame;
            CDX_LOGV("ogg->seekTimeUs %lld, pos_find:%d, opus_pos:%lld",(ogg->seekTimeUs)/1000, pos_find, opus_pos);
        }
        else
        {
            CDX_LOGW("it is not supported");
    ...
    if ((os->codec->name != NULL) && (strcasecmp(os->codec->name, "Opus") == 0))
    {
        ret = CdxStreamSeek(ogg->file, opus_pos, SEEK_SET);
    }
    else
    {
