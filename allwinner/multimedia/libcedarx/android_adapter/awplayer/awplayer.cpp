...
#endif
#if (CONF_ANDROID_MAJOR_VER >= 14)
                    bool hasHapticChannels = false;
...
status_t AwPlayer::callbackProcess(int messageId, int ext1, void* param)
{
    switch(messageId)
    {
        ...
        case SUBCTRL_SUBTITLE_AVAILABLE:
        {
            ...
            int ret = 0;
            int numOfParcel = 0;
            int maxPacketSize = SUBTITLE_MAX_PACKET_DATA_SZIE;
            unsigned int  nSubtitleId   = (unsigned int)((uintptr_t*)param)[0];
            SubtitleItem* pSubtitleItem = (SubtitleItem*)((uintptr_t*)param)[1];
            int requireBufSize = pSubtitleItem->nBitmapWidth*pSubtitleItem->nBitmapHeight*4;

            numOfParcel = (requireBufSize + maxPacketSize - 1) / maxPacketSize;
            ...
            if(mPriData->mIsSubtitleDisable == 0)
            {
                ...
                {
                    SubtitleUtilsFillTextSubtitleToParcel(&parcel[0],
                                                          pSubtitleItem,
                                                          nSubtitleId,
                ...
                else
                {
                    ...
                    ret = SubtitleUtilsFillBitmapSubtitleToParcel(&parcel[0], pSubtitleItem, nSubtitleId, 0);

                    for (int i = 1; i < numOfParcel; i++) {
                        ret += SubtitleUtilsFillBitmapSubtitleToParcel(&parcel[i], pSubtitleItem, nSubtitleId, ret);
                    }
                }
                //*record subtitile id
                ...
                // arg1 = 1 means first part of subtitle data
                sendEvent(MEDIA_TIMED_TEXT, serperate, 0, &parcel[0]);
                for (int i = 1; i < numOfParcel; i++) {
                    usleep(40 * 1000);
                    // arg1 arg2 = 01 means the packet is continue
                    // arg1 arg2 = 11 means the packet is final packet
                    if (i == numOfParcel - 1)
                        sendEvent(MEDIA_TIMED_TEXT, 1, serperate, &parcel[i]);
                    else
                        sendEvent(MEDIA_TIMED_TEXT, 0, serperate, &parcel[i]);
                }
            }
