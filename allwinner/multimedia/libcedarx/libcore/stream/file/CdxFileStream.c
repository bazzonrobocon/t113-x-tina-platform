...
repeat_read_data:
    readLen = cdxfread64(impl->fd, newBuf, len);
    totalLen += readLen;
    if (readLen < (cdx_int32)len)
    {
        if ((cdxftell64(impl->fd) - impl->offset) == impl->size) /*end of file*/
        {
            CDX_LOGD("eos, ret(%d), pos(%lld)...", readLen, impl->size);
            impl->ioErr = CDX_IO_STATE_EOS;
        }
        else
        {
            impl->ioErr = errno;
            if (readLen < 0 && impl->ioErr > 0)
            {
                impl->ioErr = CDX_IO_STATE_ERROR;
                CDX_LOGE("readLen(%d), errno(%d , %s), cur pos:(%lld), impl->size(%lld)",
                        readLen, errno, strerror(errno), cdxftell64(impl->fd) - impl->offset, impl->size);
            }
            else
            {
                newBuf = (void*)((char*)newBuf+len);
                len -= readLen;
                CDX_LOGW("readLen(%d), errno(%d , %s), cur pos:(%lld), impl->size(%lld), len(%d)",
                        readLen, errno, strerror(errno), cdxftell64(impl->fd) - impl->offset, impl->size, (cdx_int32)len);
                goto repeat_read_data;
            }
        }
    }
...
static cdx_int32 __FileStreamSeek(CdxStreamT *stream, cdx_int64 offset, cdx_int32 whence)
{
    ...
    case STREAM_SEEK_SET:
    {
        if (offset < 0 || offset > impl->size)
        {
            ...
            CdxDumpThreadStack((pthread_t)cdx_gettid());
            CdxAtomicSet(&impl->state, FILE_STREAM_IDLE);
            return -1;
        }
