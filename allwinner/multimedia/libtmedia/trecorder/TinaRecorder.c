...
static void *TRDispThread(void *param)
{
	...
	unsigned char *pVirBuf[2];
	unsigned char *pPhyBuf[2];
	int renderIndex = 0;
	 ...
	 *
	 */
	GLfloat keystone_vertex[] = { 0.0f, 0.4583f,  //left bottom
			0.0f, 0.25f,    //left top
	...
	while (hdlTina->dispportEnable) {
		...
		if (paramDisp.srcInfo.crop_w != hdlTina->vport.MainWidth
		...
		if (inputbuf->Phy_Y_addr && inputbuf->Phy_C_addr && inputbuf->Vir_Y_addr) {
#if __USE_GLES_KC__
			if (!keystoneInit) {
				alloc_VirPhyBuf(&pVirBuf[0], &pPhyBuf[0], inputbuf->bytes_used);
				alloc_VirPhyBuf(&pVirBuf[1], &pPhyBuf[1], inputbuf->bytes_used);

				//set input_texture info
				input_texture.dmafd = -1;
				input_texture.width = inputbuf->width;
				input_texture.height = inputbuf->height;
				input_texture.format = DRM_FORMAT_NV21;
				input_texture.size = inputbuf->bytes_used;

				output_texture.dmafd = -1;
				output_texture.width = inputbuf->width;
				output_texture.height = inputbuf->height;
				output_texture.format = DRM_FORMAT_NV21;
				output_texture.size = inputbuf->bytes_used;

				// create render engine
				render = renderEngineCreate(0, 0);
				if (!render) {
					TRerr("create render engine failed!\n");
				}
			...
			input_texture.phy = 0;
			input_texture.dmafd = inputbuf->fd;
			input_texture.virt = inputbuf->Vir_Y_addr;

			output_texture.dmafd = memGetBufferFd(pVirBuf[renderIndex]);
			output_texture.virt = pVirBuf[renderIndex];
			...
			memFlushCache(pVirBuf[renderIndex], hdlTina->vport.MainWidth * hdlTina->vport.MainHeight * 3 / 2);

			rBuf.isExtPhy = 0;
			rBuf.phy_addr = (unsigned long)pPhyBuf[renderIndex];
			rBuf.vir_addr = (unsigned long)pVirBuf[renderIndex];
			rBuf.y_phaddr = (unsigned long)pPhyBuf[renderIndex];
			rBuf.u_phaddr = (unsigned long)pPhyBuf[renderIndex] + inputbuf->width * inputbuf->height;
			rBuf.v_phaddr = (unsigned long)pPhyBuf[renderIndex] + inputbuf->width * inputbuf->height;
			rBuf.fd = memGetBufferFd(pVirBuf[renderIndex]);
...
#if __USE_GLES_KC__
	free_VirPhyBuf(pVirBuf[0]);
	free_VirPhyBuf(pVirBuf[1]);
...
int Tinaprepare(void *hdl)
{
	...
	if (hdlTina->status == TINA_RECORD_STATUS_DATA_SOURCE_CONFIGURED) {
		...
		if (hdlTina->vport.enable && (demoRecoder->muxType == CDX_MUXER_MOV ||
			...
			demoRecoder->videoConfig.nInputFormat = GetEncoderVideoSourceFormat(hdlTina->vport.format);
			if (demoRecoder->videoConfig.nInputFormat < 0) {
		...
		if (hdlTina->aport.enable) {
			...
			demoRecoder->audioConfig.nBitrate = hdlTina->aport.getAudioBitRate(&hdlTina->aport);
