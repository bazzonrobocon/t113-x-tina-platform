...
static int updateSdBoot0(int fd, BufferExtractCookie *cookie) {
    ...
    if (check_soc_is_secure()) {
        ...
        /*show dram param*/
        dump_dram_para((void *)newToc0Config->dram_para, 32);
    } else {
...
int burnSdBoot0(BufferExtractCookie *cookie) {
...
#if OTA_BURNBOOT0_TO_EMMC_BOOT_AREA
    ...
    if (fd > 0) {
        ...
        // update boot0 in mmcblk0boot0
        ret = writeSdBoot(fd, cookie->buffer, 0, cookie->len);
        if (ret < 0) {
            ob_error("update boot0 in mmcblk0boot0 failed! error is %d : %s\n",
                    errno, strerror(errno));
            return ret;
        }
        ob_debug("update boot0 in mmcblk0boot0 succeed! writed %d bytes\n", ret);
