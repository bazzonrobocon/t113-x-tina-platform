...
static const int DEMUX_COMMAND_SET_VIDEO_INDEX           = 0X113;
...
typedef struct DemuxCompContext_t
{
    ...
    sem_t                       semSetVideoIndex;

    int                         nSetDataSourceReply;
    int                         nStartReply;
    int                         nStopReply;
    int                         nSetVideoIndexReply;
...
int DemuxCompSetVideoIndex(DemuxComp* d, int index)
{
    AwMessage msg;
    DemuxCompContext* demux;
    ...
    //* send a prepare message.
    memset(&msg, 0, sizeof(AwMessage));
    msg.messageId = DEMUX_COMMAND_SET_VIDEO_INDEX;
    msg.params[0] = (uintptr_t)&demux->semSetVideoIndex;
    msg.params[1] = (uintptr_t)&demux->nSetVideoIndexReply;
    msg.params[2] = index;

    AwMessageQueuePostMessage(demux->mq, &msg);
    SemTimedWait(&demux->semSetVideoIndex, -1);

    return demux->nSetVideoIndexReply;
}
...
MediaInfo* DemuxCompGetMediaInfo(DemuxComp* d)
{
    ...
    memcpy(&mi->mId3Info, &myMediaInfo->mId3Info, sizeof(CdxMediaInfoT));
...
_endTask:
        ...
        else if(msg.messageId == DEMUX_COMMAND_SET_VIDEO_INDEX)
        {
            int index = (int)msg.params[2];
            CdxParserControl(demux->pParser, CDX_PSR_CMD_SET_VIDEO_INDEX, (void*)&index);
            if(pReplyValue != NULL)
                *pReplyValue = 0;
            if(pReplySem != NULL)
                sem_post(pReplySem);
        }//* end DEMUX_COMMAND_SET_VIDEO_INDEX.
...
static int setMediaInfo(MediaInfo* pMediaInfo, CdxMediaInfoT* pInfoFromParser)
{
    ...
    memcpy(&pMediaInfo->mId3Info, pInfoFromParser, sizeof(CdxMediaInfoT));
